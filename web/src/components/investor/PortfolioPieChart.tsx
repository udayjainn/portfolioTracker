...
import { securityChartLabel } from "@/lib/securityDisplay";
...
export function PortfolioPieChart({ holdings }: { holdings: HoldingSummary[] }) {
  ...
  const restPct = rest.reduce((sum, h) => sum + (toNumber(h.pct_of_portfolio) ?? 0), 0);

  const chartData = [
    ...top.map((h) => ({
      name: securityChartLabel(h.security),
      value: toNumber(h.pct_of_portfolio) ?? 0,
      usd: toNumber(h.value_usd) ?? 0,
    })),
    ...(rest.length > 0
      ? [{ name: "Other", value: restPct, usd: rest.reduce((s, h) => s + (toNumber(h.value_usd) ?? 0), 0) }]
      : []),
  ];
